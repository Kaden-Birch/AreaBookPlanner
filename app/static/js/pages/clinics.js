...
import { esc, attr, dot, fmtDate, fmtDateOnly, relativeDays, badge, options, navigate, debounce, setTitle, COLOR_ORDER } from '../ui.js';
import { openClinicForm } from '../forms.js';

let state = { q: '', relationship: '', color: '', sort: 'name' };
...
export async function render(container) {
  ...
  ['relationship', 'color', 'sort'].forEach(k => {
...
async function load() {
  const list = await clinics.list({ q: state.q, relationship: state.relationship, color: state.color });
  ...
  for (const c of sorted) {
    if (state.sort === 'name') {
      ...
      if (group !== lastLetter) { rows += `<tr class="group-row"><td colspan="8">${group}</td></tr>`; lastLetter = group; }
    }
    rows += `
  ...
  el.innerHTML = `
    <table class="table">
      <thead><tr>
        <th>Clinic</th><th>Status</th><th>Type</th><th>Address</th><th>Phone</th><th>Last visit</th><th>Next appt</th><th>Follow-up</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
