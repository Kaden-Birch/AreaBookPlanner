...
async function run(q) {
  ...
  add('Equipment', r.devices || [], d => ({ url: `#/clinics/${d.clinic_id}/equipment?device=${d.id}`, html: `${esc(d.icon)} <strong>${esc(d.name)}</strong><span class="muted"> · ${[d.user_name, d.ip_address].filter(Boolean).map(esc).join(' · ')}${d.clinic_name ? ' · ' + esc(d.clinic_name) : ''}</span>` }));
