...
export async function importSyncro(){
  ...
  try{
    ...
    function review(data){
      ...
      const fill=document.createElement('label');fill.innerHTML='<input type="checkbox" name="fill-network"> Fill missing network information on previously imported machines (only when there are no interfaces or existing IP/MAC values). Existing network documentation is never changed.';preview.querySelector('[data-confirm]').before(fill);
