    ...
    function review(data){
      ...
      preview.querySelectorAll('pre').forEach(pre=>{
        ...
        details.innerHTML=Object.entries(record).filter(([key,value])=>key!=='id'&&key!=='url'&&value!==''&&value!==null).map(([key,value])=>{
          ...
          const content=key==='addresses'?value.map(a=>a.address+(a.prefix===null?'':'/'+a.prefix)).join(', '):Array.isArray(value)?value.join(', '):String(value);
          return `<dt>${esc(label)}</dt><dd>${esc(content||'Not recorded')}</dd>`;
        }).join('');pre.replaceWith(details);
      });
      preview.querySelector('[data-confirm]').onclick=async e=>{e.target.disabled=true;try{const result=await api.post('/api/syncro/import',{token:data.token,name:preview.querySelector('[name=name]').value,area_id:Number(preview.querySelector('[name=area]').value),clinic_id:Number(preview.querySelector('[name=clinic]').value)||null});modal.close();toast(`Syncro: ${result.created} new records, ${result.preserved} local records preserved.`,'success');navigate('#/clinics/'+result.clinic_id);}catch(error){status.textContent=error.message;e.target.disabled=false;}};
    }
    await search();
