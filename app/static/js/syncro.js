...
export async function syncroSettings(container){
  ...
  (await import('./syncro-connections.js')).connectionSettings(container);
...
export async function importSyncro(initialClinic=null){
  ...
  try{
    const [areas,clinics,connections]=await Promise.all([api.get('/api/admin/areas'),api.get('/api/clinics'),api.get('/api/syncro/connections')]);
    modal.body.innerHTML=`<label>Syncro connection<select name="connection">${connections.map(c=>`<option value="${c.id||''}">${esc(c.name)} · ${esc(c.subdomain)}${c.configured?'':' (key missing)'}</option>`).join('')}</select></label><p>Several Syncro customers can share one clinic/site and its UniFi or Meraki network. Review before importing. No writes are sent to Syncro. Existing local records are preserved; source snapshots refresh on reimport.</p><div class="actions"><input name="query" aria-label="Search Syncro customers" placeholder="Customer name"><button class="btn" data-search>Search</button><button class="btn" data-next>Next page</button></div><p data-status role="status"></p><div data-results></div><div data-preview></div>`;
    let page=1;
    const connection=()=>Number(modal.body.querySelector('[name=connection]').value)||null;
    modal.body.querySelector('[name=connection]').onchange=()=>{page=1;preview.innerHTML='';search();};
    ...
    async function search(){status.textContent='Reading Syncro…';try{const data=await api.get('/api/syncro/customers',{query:modal.body.querySelector('[name=query]').value,page,connection_id:connection()});results.innerHTML=data.customers.map(c=>`<p><button class="btn" data-customer="${c.id}">${esc(c.name)} · ${esc(c.city)}${c.clinic_id?' · Already linked':''}</button></p>`).join('')||'<p>No customers found.</p>';status.textContent=`Page ${page}`;results.querySelectorAll('[data-customer]').forEach(b=>b.onclick=()=>choose(Number(b.dataset.customer)));}catch(e){status.textContent=e.message;}}
    ...
    function choose(id){
      ...
      preview.querySelector('[data-build]').onclick=async e=>{e.target.disabled=true;status.textContent='Building reviewed import…';try{const data=await api.post('/api/syncro/preview',{customer_id:id,connection_id:connection(),categories:[...preview.querySelectorAll('[name=category]:checked')].map(i=>i.value)});review(data);}catch(error){status.textContent=error.message;e.target.disabled=false;}};
    }
    function review(data){
      status.textContent='Preview ready. Nothing has been imported.';
      const list=Array.isArray(clinics)?clinics:clinics.clinics||[];
      preview.innerHTML=`<h3>Review import</h3><label>Clinic name<input name="name" value="${attr(data.clinic.name)}"></label><p>${esc([data.clinic.address,data.clinic.city,data.clinic.province,data.clinic.postal_code,data.clinic.phone,data.clinic.email].filter(Boolean).join(' · '))}</p><label>Area<select name="area">${areas.filter(a=>a.is_active).map(a=>`<option value="${a.id}">${esc(a.name)}</option>`).join('')}</select></label><label>Match existing clinic<select name="clinic" ${data.clinic_id?'disabled':''}><option value="">Create a new current-client clinic</option>${list.map(c=>`<option value="${c.id}" ${c.id===(data.clinic_id||initialClinic)?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><p>Check for an existing clinic before creating one. Select the same clinic and site for attached practices or pharmacies sharing a network. Existing clinic details and network mappings are not replaced. Invoice history stays separate from AreaBook billing.</p>${data.warnings.map(w=>`<p class="text-danger">${esc(w)}</p>`).join('')}${Object.entries(data.records).map(([kind,rows])=>`<details><summary>${esc(kind)}: ${rows.length} records</summary>${rows.map(r=>`<pre style="white-space:pre-wrap">${esc(JSON.stringify(r,null,2))}</pre>`).join('')}</details>`).join('')}<button class="btn btn-primary" data-confirm>Confirm import</button>`;
      const chosen=list.find(c=>c.id===(data.clinic_id||initialClinic));if(chosen)preview.querySelector('[name=area]').value=chosen.area_id;
      const site=document.createElement('label');site.innerHTML='AreaBook site<select name="location"><option value="">Main Site</option></select>';preview.querySelector('[name=clinic]').parentElement.after(site);
      async function sites(){
        const cid=Number(preview.querySelector('[name=clinic]').value)||null;
        const clinic=list.find(c=>c.id===cid);if(clinic)preview.querySelector('[name=area]').value=clinic.area_id;
        const confirm=preview.querySelector('[data-confirm]');confirm.disabled=true;
        try{const rows=cid?await api.get(`/api/clinics/${cid}/locations`):[];site.querySelector('select').innerHTML='<option value="">Main Site</option>'+rows.map(l=>`<option value="${l.id}">${esc(l.name)}</option>`).join('');site.querySelector('select').value=data.clinic_id?(data.location_id||''):'';site.querySelector('select').disabled=Boolean(data.clinic_id);confirm.disabled=false;}catch(e){status.textContent=e.message;}
      }
      preview.querySelector('[name=clinic]').onchange=sites;sites();
