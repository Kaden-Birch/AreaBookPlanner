...
import { displayGraph, layoutGraph, nodeHeight } from './topology-graph.js';
...
export function mountTopology(body, topo, meta, key, actions) {
  ...
  const save = () => { try { localStorage.setItem(key, JSON.stringify({ hidden: [...hidden], collapsed: [...collapsed],orientation,vlans:[...selectedVlans],vlanOnly })); } catch { /* Browser storage may be disabled. */ } };
  ...
  const fit = () => {
    ...
    const points=[...[...positions].map(([id,p])=>({...p,h:nodeHeight(byId.get(id))})),...vpnPositions.map(p=>({...p,h:80}))];
  ...
  const draw = (refit=true) => {
    ...
    graph=displayGraph(all,topo.edges,[...hidden],[...collapsed],vlanHidden); positions=layoutGraph(graph.nodes,graph.edges,orientation);
